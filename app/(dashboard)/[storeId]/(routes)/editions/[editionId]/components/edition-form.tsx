...
import { Edition, Category, PublishingHouse, Image, Book, Language } from "@prisma/client"
...
const formSchema = z.object({
  ...
  publishingHouseId: z.string().min(1),
...
export const EditionForm: React.FC<EditionFormProps> = ({
  initialData,
  categories,
  publishingHouses,
  books,
  ...
  const onSubmit = async (data: EditionFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/editions/${params.editionId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/editions`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/editions`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/editions/${params.editionId}`);
      router.refresh();
      router.push(`/${params.storeId}/editions`);
      toast.success('Editions deleted.');
    } catch (error: any) {
      toast.error('Something went wrong.');
    } finally {
      setLoading(false);
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue defaultValue={field.value} placeholder="Select a book" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {books.map((book) => (
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue defaultValue={field.value} placeholder="Select a language" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue defaultValue={field.value} placeholder="Select a publishing house" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {publishingHouses.map((publishingHouse) => (
                        <SelectItem key={publishingHouse.id} value={publishingHouse.id}>{publishingHouse.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <FormMessage />
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <FormDescription>
                      This edition will appear on the home page
                    </FormDescription>
                  </div>
                </FormItem>
              )}
