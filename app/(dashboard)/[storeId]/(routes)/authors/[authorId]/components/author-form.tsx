...
import { Book, Writer, Author } from "@prisma/client"
...
export const AuthorForm: React.FC<AuthorFormProps> = ({
  ...
  const onSubmit = async (data: AuthorFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/authors/${params.authorId}`, data);
      } else {
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue defaultValue={field.value} placeholder="Select a writer" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {writers.map((writer) => (
                        <SelectItem key={writer.id} value={writer.id}>{writer.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <FormMessage />
